...
@router.message(SupportState.user_type, F.text.lower().in_(['отправитель', 'курьер']))
...
async def handle_unknown_problem(message: Message, state: FSMContext):
    await message.answer('Пожалуйста, выберите проблему из списка', reply_markup=kb.support_problems_reply_mu)
...
async def handle_other_problem_description(message: Message, state: FSMContext):
    ...
    await message.answer('Укажите номер заявки, по которому возникла проблема\n (Номер заявки можно найти в сообщении, подтверждающем создание заявки)', reply_markup=ReplyKeyboardRemove())
...
async def handle_request_no(message: Message, state: FSMContext):
    ...
    if not request_data:
        await message.answer('Заявка с таким номером не найдена. Пожалуйста, проверьте номер заявки и попробуйте еще раз')
        return

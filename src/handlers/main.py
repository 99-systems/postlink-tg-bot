...
async def start(message: Message, state: FSMContext):
    ...
    welcome_message = (
        ...
        "📦 *Отправителю* — быстрый и удобный способ передать посылку.\n"
        "✈️ *Путешественнику* — возможность заработать в пути.\n\n"
        "> *Как это работает:*\n"
        "> 1️⃣ Выбираешь направление\n"
        "> 2️⃣ Создаёшь заявку или откликаешься на чужую\n"
        "> 3️⃣ Получаешь отклик и договариваешься в чате\n\n"
        "🚀 Всё просто: открой приложение и проверь всё сам 👇🏻"
    )

    await message.answer(welcome_message, reply_markup=simple_kb, parse_mode="MarkdownV2")
